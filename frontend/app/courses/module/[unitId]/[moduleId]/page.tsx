...
export default function ModulePage({
  ...
  params: { courseId: string; unitId: string; moduleId: string }
}) {
  ...
  const module = {
    ...
    content: `
      ...
      <p>Before you can start writing Python code, you need to set up your development environment. This module will guide you through installing Python and setting up a code editor.</p>
...
sudo pacman -S python         # For Arch Linux</code></pre>
      ...
      <h3>Conclusion</h3>
      <p>Now that you have Python installed and a code editor set up, you're ready to start writing Python code. In the next module, we'll explore basic Python syntax and write our first program.</p>
    `,
    video: {
    ...
    quiz: [
      ...
      {
        question: "Which of the following is NOT a Python IDE or code editor mentioned in the module?",
    ...
    unit: {
      ...
      progress: 20,
      modules: [
        { id: "1", title: "Installing Python and Setting Up the Development Environment" },
        { id: "2", title: "Installing Python and Understanding the Interpreter" },
        { id: "3", title: "Python Installation and Environment Setup Quiz" },
        { id: "4", title: "Write Your First Python Script" },
        { id: "5", title: "Video Lecture: Learn Python - Full Course for Beginners" },
      ],
    },
  }
  ...
  const currentModuleIndex = module.unit.modules.findIndex((m) => m.id === params.moduleId)
  const prevModule = currentModuleIndex > 0 ? module.unit.modules[currentModuleIndex - 1] : null
  const nextModule =
    currentModuleIndex < module.unit.modules.length - 1 ? module.unit.modules[currentModuleIndex + 1] : null
    ...
    <AppShell>
      <div className="flex flex-col">
          ...
          <div className="container mx-auto px-4">
            <div className="flex flex-col gap-2">
              <div className="flex items-center gap-2 text-sm text-muted-foreground">
                <Link href={`/courses/${params.courseId}`} className="hover:text-foreground transition-colors">
                  {module.course.title}
                </Link>
                <span>/</span>
                <span>{module.unit.title}</span>
              </div>
              <h1 className="text-xl md:text-2xl font-display">{module.title}</h1>
        ...
        <div className="h-1 bg-muted">
          <div className="h-full bg-primary transition-all" style={{ width: `${module.unit.progress}%` }}></div>
        </div>
          ...
          <div className="flex flex-col lg:flex-row gap-6">
            ...
            <div className="hidden lg:block lg:w-64 shrink-0">
              <div className="sticky top-24 space-y-4">
                <h3 className="font-medium">Unit Modules</h3>
                <ul className="space-y-1">
                  {module.unit.modules.map((m, index) => (
                    <li key={index}>
                      <Link
                        href={`/courses/${params.courseId}/module/${params.unitId}/${m.id}`}
                        className={`flex items-center gap-2 py-2 px-3 rounded-md text-sm ${
                          m.id === params.moduleId ? "bg-primary/10 text-primary font-medium" : "hover:bg-muted/50"
                        }`}
                      >
              ...
              <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-6">
                <TabsList className="w-full max-w-md mx-auto">
                  <TabsTrigger value="content" className="flex-1">
                    {module.type === "video" ? "Video" : "Content"}
                  </TabsTrigger>
                    ...
                    <div className="flex justify-between pt-4 border-t border-border/40">
                      {prevModule ? (
                        <Button variant="outline" asChild>
                          <Link href={`/courses/${params.courseId}/module/${params.unitId}/${prevModule.id}`}>
                      ...
                      {nextModule ? (
                        <Button className="glow-button" asChild>
                          <Link href={`/courses/${params.courseId}/module/${params.unitId}/${nextModule.id}`}>
                            Next Module
                  ...
                  <div className="max-w-3xl mx-auto space-y-8">
                    <div className="bg-muted/20 p-6 rounded-lg border border-border/40">
                      ...
                      <p className="mb-6 text-muted-foreground">
                        Test your understanding of the module content with these questions.
                      </p>
      ...
      <div className="lg:hidden fixed bottom-0 left-0 right-0 bg-background border-t border-border/40 p-4">
        <Button variant="outline" size="sm" className="w-full flex items-center justify-between" asChild>
          <Link href={`/courses/${params.courseId}`}>
            ...
            <span>
              {currentModuleIndex + 1}/{module.unit.modules.length}
            </span>
          </Link>
        </Button>
      </div>
