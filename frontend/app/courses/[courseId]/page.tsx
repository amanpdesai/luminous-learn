...
export default function CoursePage({ params }: { params: { courseId: string } }) {
  ...
  // Function to get the appropriate icon for module type
    ...
    <AppShell>
      <div className="flex flex-col min-h-screen">
        ...
        <div className="flex-1 container mx-auto px-4 py-8 lg:flex lg:gap-8">
          {/* Main content area - Course units and modules */}
          <div className="lg:flex-1">
            <div className="mb-6">
              <h2 className="text-xl font-display mb-4">Course Content</h2>
              <div className="flex justify-between text-sm text-muted-foreground mb-2">
                <span>
                  {course.units.length} units • {course.units.reduce((acc, unit) => acc + unit.totalModules, 0)} modules
                </span>
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div className="text-left">
                        ...
                        <p className="text-xs text-muted-foreground">
                          {unit.modules.length} modules • {unit.completedModules} completed
                        </p>
                      </div>
                    </div>
                    <ChevronDown
                  ...
                  {expandedUnits[unitIndex] && (
                    <div className="border-t border-border/30">
                      {unit.modules.map((module, moduleIndex) => (
                        <Link
                          href={`/courses/${course.id}/module/${unit.id}/${module.id}`}
                          key={moduleIndex}
