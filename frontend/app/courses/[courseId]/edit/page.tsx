  ...
  useEffect(() => {
    ...
    const fetchCourse = async () => {
      ...
      try {
        ...
        setHoursPerWeek(data.estimated_duration_hours_per_week?.toString() || "")
        setNumberOfWeeks(data.estimated_number_of_weeks?.toString() || "")
        setCourseLevel(data.level || "")
        setCourseDepth(data.depth || "")
        setUnits(data.units || [])
  
        originalData.current = data // Save original data for comparison
        setIsLoading(false)
      } catch (err) {
    ...
    const handlePublish = async () => {
      ...
      try {
        ...
        if (!deleteResponse.ok) {
          console.warn("Note: Draft was not deleted, but course was published successfully", deleteResponse)
        } else {
